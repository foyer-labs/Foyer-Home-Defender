...
- **An armed house keeps the answer it was armed with.** While any area is
  ...
  disarm, and the radios and their thresholds cannot be changed: disarm
  first. Who may command the house stays editable: a guest's code, a lost
  phone's rule or a keypad's token can be taken away from anywhere, whatever
  is armed — except the last code anybody can use, or a validity window that
  would end it sooner, because with none the code policy switches itself
  off.
