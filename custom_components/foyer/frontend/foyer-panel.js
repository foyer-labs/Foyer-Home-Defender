...
var gt = {
	...
	scenario_id: null,
	enabled: !0
}, _t = ["tag.", "event."], vt = class extends R {
	constructor(...e) {
        ...
        ${n.length ? k`<div class="table-wrap">
              <table>
                <thead>
                  <tr>
                    <th>${H(t, "field.name")}</th>
                    <th>${H(t, "field.kind")}</th>
                    <th>${H(t, "devices.reaches")}</th>
                    <th>${H(t, "devices.identifies")}</th>
                    <th>${H(t, "field.enabled")}</th>
                  </tr>
                </thead>
                <tbody>
                  ${n.map((e) => this._row(t, e))}
            ...
            </div>` : k`<div class="empty">${H(t, "devices.none")}</div>`}
        <div class="card-bd">
          <p class="note">${H(t, "devices.white_list")}</p>
        </div>
      </div>
      ${this._draft ? this._renderEditor(t, this._draft) : j}
      ${this._renderMqtt(t)}
    ...
    >
      ...
      <td>${H(e, `device_kind.${t.kind}`)}</td>
      <td class="mono">${t.kind === "keypad" ? t.ref : t.entity_id}</td>
      <td>
        ${t.kind === "tag" ? k`<span class="pill ok">${n?.name ?? "—"}</span>` : k`<span class="pill idle">${H(e, "devices.code_is_identity")}</span>`}
      </td>
      <td>${H(e, t.enabled ? "common.yes" : "common.no")}</td>
    </tr>`;
	}
	_renderEditor(e, t) {
      ...
      <div class="card">
        <div class="card-hd">
          <h2>${t.id ? t.name : H(e, "devices.new")}</h2>
        </div>
        <div class="card-bd">
          <div class="grid-form">
            <label class="field">
            ...
            <label class="field">
              <span class="lbl">${H(e, "field.kind")}</span>
              <select
                @change=${(e) => this._setKind(e.target.value)}
              >
                ${["keypad", "tag"].map((n) => k`<option .value=${n} ?selected=${n === t.kind}>
                    ${H(e, `device_kind.${n}`)}
                  </option>`)}
              </select>
              <span class="hint">${H(e, `devices.kind_hint_${t.kind}`)}</span>
            </label>
          </div>

          ${t.kind === "keypad" ? k`<div class="grid-form">
                <label class="field">
                  <span class="lbl">${H(e, "field.ref")}</span>
                  <input
                    .value=${t.ref ?? ""}
                    placeholder="keypad_hall"
                    @input=${(e) => this._set("ref", e.target.value)}
                  />
                  <span class="hint">${H(e, "devices.ref_hint")}</span>
                </label>
              </div>` : k`
                <div class="banner warn">
                  <strong>${H(e, "devices.stolen_tag")}</strong>
                  <span>${H(e, "devices.stolen_tag_hint")}</span>
                </div>
                <div class="grid-form">
                  <label class="field">
                    <span class="lbl">${H(e, "field.entity_id")}</span>
                    ...
                    >
                      ...
                      ${a.map((e) => k`<option .value=${e} ?selected=${e === t.entity_id}>
                          ${e}
                        </option>`)}
                    </select>
                    <span class="hint">${H(e, "devices.entity_hint")}</span>
                  </label>
                  <label class="field">
                    <span class="lbl">${H(e, "field.event_type")}</span>
                    <input
                      .value=${t.event_type ?? ""}
                      @input=${(e) => this._set("event_type", e.target.value || null)}
                    />
                    <span class="hint">${H(e, "devices.event_type_hint")}</span>
                  </label>
                  <label class="field">
                    <span class="lbl">${H(e, "field.user_id")}</span>
                    <select
                      @change=${(e) => this._set("user_id", e.target.value || null)}
                    >
                      <option value="" ?selected=${!t.user_id}>—</option>
                      ${r.map((e) => k`<option .value=${e.id ?? ""} ?selected=${e.id === t.user_id}>
                          ${e.name}
                        </option>`)}
                    </select>
                    <span class="hint">${H(e, "devices.owner_hint")}</span>
                  </label>
                  <label class="field">
                    <span class="lbl">${H(e, "field.command")}</span>
                    <select
			...
			"disarm"
                  ...
                  ${t.command === "disarm" ? j : k`<label class="field">
                        <span class="lbl">${H(e, "field.scenario_id")}</span>
                        <select
          ...
          <label class="check">
            ...
            <span>
              ${H(e, "field.enabled")}
              <span class="hint">${H(e, "devices.enabled_hint")}</span>
            </span>
          </label>
	...
	_renderMqtt(e) {
      ...
      <div class="card">
        <div class="card-hd">
          <h2>${H(e, "devices.mqtt")}</h2>
        </div>
        <div class="card-bd">
          <p class="note">${H(e, "devices.mqtt_note")}</p>
          <label class="check">
            <input
              type="checkbox"
              .checked=${t.enabled}
            ...
            />
            <span>
              ${H(e, "devices.mqtt_enabled")}
              <span class="hint">${H(e, "devices.mqtt_enabled_hint")}</span>
            </span>
          </label>
          <div class="grid-form">
            <label class="field">
              <span class="lbl">${H(e, "field.command_topic")}</span>
              <input
                .value=${t.command_topic}
                placeholder=${H(e, "devices.topic_command_example")}
                @input=${(e) => n("command_topic", e.target.value)}
              />
              <span class="hint">${H(e, "devices.topic_hint")}</span>
            </label>
            <label class="field">
              <span class="lbl">${H(e, "field.state_topic")}</span>
              <input
                .value=${t.state_topic}
                placeholder=${H(e, "devices.topic_state_example")}
                @input=${(e) => n("state_topic", e.target.value)}
              />
            </label>
            <label class="field">
              <span class="lbl">${H(e, "field.detail")}</span>
              <select
			...
			"standard",
			"full"
		].map((n) => k`<option .value=${n} ?selected=${n === t.detail}>
                    ${H(e, `mqtt_detail.${n}`)}
                  </option>`)}
              </select>
              <span class="hint">${H(e, `devices.detail_hint_${t.detail}`)}</span>
            </label>
            <label class="field">
              <span class="lbl">${H(e, "field.qos")}</span>
              <select
			...
			1,
			2
		].map((e) => k`<option .value=${String(e)} ?selected=${e === t.qos}>
                    ${e}
                  </option>`)}
              </select>
            </label>
          ...
          <label class="check">
            ...
            <span>
              ${H(e, "field.retain")}
              <span class="hint">${H(e, "devices.retain_hint")}</span>
            </span>
          </label>
          <div class="hr"></div>
          <div class="grid-form">
            <div class="field">
              <span class="lbl">${H(e, "devices.inbound")}</span>
              <pre class="sample">${yt}</pre>
            </div>
            <div class="field">
              <span class="lbl">${H(e, "devices.outbound")}</span>
              <pre class="sample">${bt[t.detail]}</pre>
              <span class="hint">${H(e, "devices.last_result_hint")}</span>
            </div>
          </div>
          ${this._mqttProblems.length ? k`<ul class="problems">
                ${this._mqttProblems.map((t) => k`<li>${G(e, t)}</li>`)}
              </ul>` : j}
        </div>
        <div class="card-ft">
          <button
...
var Z = 50, xt = class extends R {
	...
	_renderRow(e, t) {
      ...
      <tr class="clickable" aria-selected=${a ? "true" : "false"} @click=${() => this._open = a ? void 0 : t.id}>
        ...
        <td>
          <span class="state ${wt(t.severity)}">
            ${St(e, t.event_type)}
          </span>
        </td>
        ...
        <td>${o}</td>
        <td>
          ${t.user_name ?? (t.channel ? Ct(e, t.channel) : "")}
          ${t.detail?.attributed === "claimed" ? k`<span class="claimed">${H(e, "log.claimed")}</span>` : j}
        </td>
...
var Tt = ["en", "it"], Et = {
	...
	get _chime() {
		return this._draft ?? structuredClone(this.ctx?.config?.chime ?? Et);
	}
	_set(e, t) {
		this._draft = {
			...this._chime,
        ...
        <div class="card-bd">
          <label class="field">
            ...
            >
              ...
              ${Tt.map((t) => k`<option .value=${t} ?selected=${t === n.language}>
                    ${H(e, `language.${t}`)}
                  </option>`)}
            </select>
...
customElements.get("foyer-page-settings") || customElements.define("foyer-page-settings", Dt);
//#endregion
...
customElements.get("foyer-wizard") || customElements.define("foyer-wizard", kt);
//#endregion
...
var Pt = class extends R {
	...
	_renderTabs(e) {
		let t = this._canConfigure ? At : At.filter((e) => !jt.includes(e));
	...
	_renderPage(e, t) {
		switch (this._tick, e) {
			...
			case "devices": return k`<foyer-page-devices .ctx=${t}></foyer-page-devices>`;
...
customElements.get("foyer-panel") || customElements.define("foyer-panel", Pt);
//#endregion
