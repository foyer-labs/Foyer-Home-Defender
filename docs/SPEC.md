...
| 167 | Every frontend element is defined through `shared/define.ts`, which waits until the Home Assistant app has defined `<home-assistant>`; the card joins the picker's list only after its definition | The app replaces the custom element registry (the scoped registry polyfill) when it starts, and the card, loaded as an extra module on every page, could run first: defined in the old registry it was listed in the picker and could not be created, so it could not be added from the UI. Reproduced on a local Home Assistant 2026.6, where the card was undefined before the change and addable after |
